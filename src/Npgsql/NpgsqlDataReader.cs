...
slow:
    ...
    internal async Task Cleanup(bool async, bool connectionClosing = false, bool isDisposing = false)
    {
        ...
        Buffer = null!;
        // Reset reader so the next command doesn't inherit our setup.
        if (!Connector.IsBroken)
            Connector.ReadBuffer.PgReader.Reset();
    ...
    public override long GetChars(int ordinal, long dataOffset, char[]? buffer, int bufferOffset, int length)
    {
        ...
        var format = GetInfo(ordinal, typeof(GetChars), out var converter, out var bufferRequirement, out var asObject);
    ...
    public override Task<T> GetFieldValueAsync<T>(int ordinal, CancellationToken cancellationToken)
    {
        ...
        return _isRowBuffered ? Task.FromResult(GetFieldValueCore<T>(ordinal)) : Core(ordinal, cancellationToken);
    ...
    public override bool IsDBNull(int ordinal)
        => SeekToColumn(ordinal, CheckRowAndGetField(ordinal).DataFormat, resumableOp: true) is -1;
    ...
    public override Task<bool> IsDBNullAsync(int ordinal, CancellationToken cancellationToken)
    {
        ...
        {
            using var registration = Connector.StartNestedCancellableOperation(cancellationToken, attemptPgCancellation: false);
            return await SeekToColumnAsync(ordinal, CheckRowAndGetField(ordinal).DataFormat, resumableOp: true).ConfigureAwait(false) is -1;
        }
    }
